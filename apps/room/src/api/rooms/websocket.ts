...
websocket.get(
  ...
  createWebSocketMiddleware(async (c) => {
    ...
    const { type, user, room, metadata } = (await verify(authorization.trim(), env.JwtSecret)) as MatchmakingDataJWT;
    if (type !== "matchmaking" || room.server.id !== env.ServerId) return;
