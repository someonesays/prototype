<main>
  <p><a class="url" href="/">Back</a></p>
  
  <h1>Credits</h1>

  <h2>Developers</h2>
  <ul>
    <li>Two: <a class="url" href="https://github.com/real2two" target="_blank">https://github.com/real2two</a></li>
  </ul>
  
  <h2>Logo designer</h2>
  <ul>
    <li>Realmy: <a class="url" href="https://realmy.net/" target="_blank">https://realmy.net/</a></li>
  </ul>

  <h2>External assets used</h2>
  <ul>
    <li>Figtree: <a class="url" href="https://www.erikdkennedy.com/projects/figtree.html" target="_blank">https://www.erikdkennedy.com/projects/figtree.html</a></li>
    <li>Fontawesome: <a class="url" href="https://fontawesome.com/" target="_blank">https://fontawesome.com/</a></li>
  </ul>
</main>
